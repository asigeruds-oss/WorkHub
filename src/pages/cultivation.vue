  ...
  <v-container>
          ...
          <v-col cols="12" md="6">
            <v-card class="cultivation-status-card">
              <v-card-title class="d-flex align-center">
                ...
                <div>
                  <div class="text-h6">
                    {{ userCultivation.realm_name }} ·
                    {{ userCultivation.stage_name }}
                  </div>
                  <div class="text-caption">
                    修炼者：{{ userCultivation.username }}
                  </div>
                </div>
                <v-spacer></v-spacer>
                <v-btn
                    ...
                    <v-card variant="outlined" class="pa-3 resource-card">
                      <div class="d-flex align-center">
                        ...
                        <div>
                          ...
                          <div class="text-h6">
                            {{ userCultivation.energy }} /
                            {{ userCultivation.max_energy }}
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="text-right mt-2">
                        <v-btn
                          ...
                          variant="text"
                          :disabled="
                            userCultivation.spirit_stones < 5 ||
              ...
              <v-card-text v-if="userStats">
                <v-list>
                  ...
                  <v-list-item>
                    ...
                    <template v-slot:append>
                      <span class="text-body-1">{{
                        userStats.high_priority_completed
                      }}</span>
                    </template>
                  </v-list-item>
                  ...
                  <v-list-item>
                    ...
                    <template v-slot:append>
                      <span class="text-body-1">{{
                        userStats.continuous_active_days
                      }}</span>
                    </template>
                  </v-list-item>
                  ...
                  <v-list-item>
                    ...
                    <template v-slot:append>
                      <span class="text-body-1">{{
                        userStats.max_continuous_active_days
                      }}</span>
                    </template>
                  </v-list-item>
                </v-list>
    ...
    <v-dialog v-model="recoverEnergyDialog" max-width="500">
      <v-card>
        ...
        <v-card-text>
          <p>
            当前灵石: <strong>{{ userCultivation?.spirit_stones || 0 }}</strong>
          </p>
          <p class="mb-4">
            消耗 <strong>5</strong> 灵石可恢复 <strong>10</strong> 点灵力。
          </p>
...
const nextStageExp = computed(() => {
  if (
    !userCultivation.value ||
    !currentStages.value ||
    currentStages.value.length === 0
  ) {
    return null;
  }
...
onMounted(async () => {
  try {
    ...
    // 显示欢迎消息
    showMessage(
      `欢迎回来，${userCultivation.value.username}！当前境界：${userCultivation.value.realm_name} · ${userCultivation.value.stage_name}`,
      "info"
    );
  } catch (err) {
...
function getRealmIconUrl() {
  ...
  const realm = realms.value.find(
    (r) => r.id === userCultivation.value.current_realm
  );
  if (!realm || !realm.icon) {
    return null;
  }
...
async function tryAdvanceCultivation() {
  ...
  try {
    ...
    if (
      result.cultivation &&
      result.cultivation.current_realm !== userCultivation.value.current_realm
    ) {
      await cultivationStore.fetchStagesByRealm(
        result.cultivation.current_realm
      );
    }
  } catch (err) {
...
async function handleCompleteTodo(todoId) {
  try {
    ...
    if (result.rewards) {
      showMessage(
        `完成待办！获得 ${result.rewards.exp} 修为和 ${result.rewards.spirit_stones} 灵石`,
        "success"
      );
    } else {
