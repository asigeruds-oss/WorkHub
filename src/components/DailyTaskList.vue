<template>
  <v-card class="mb-4" elevation="3" rounded="lg">
    ...
    <v-card-text v-else-if="dailyTasks.length === 0" class="text-center pa-6">
      <v-icon icon="mdi-calendar-blank" size="large" class="mb-2 text-grey-lighten-1"></v-icon>
          ...
          <div class="d-flex align-center">
            ...
            <v-menu>
              ...
              <v-list>
                ...
                <v-list-item v-if="!task.is_completed_today" @click="completeTask(task)" density="compact">
                  <template v-slot:prepend>
                ...
                <v-list-item v-if="task.is_completed_today" @click="cancelCompleteTask(task)" density="compact">
                  <template v-slot:prepend>
    ...
    <v-dialog v-model="dialog" max-width="500" transition="dialog-bottom-transition">
      <v-card rounded="lg">
        ...
        <v-card-text class="pt-4">
          <v-form ref="form" @submit.prevent="isEditing ? updateTask() : addTask()">
            <v-text-field
    ...
    <v-dialog v-model="deleteDialog" max-width="400" transition="dialog-top-transition">
      <v-card rounded="lg">
        ...
        <v-card-text class="pt-4 pb-2 text-center">
          ...
          <p class="text-subtitle-1 font-weight-medium mt-2">"{{ currentTask.title }}"</p>
...
async function toggleTaskStatus(task) {
  try {
    ...
    console.log(`[DailyTaskList] 切换日常任务状态，ID: ${task.id}, 当前完成状态: ${task.is_completed_today}`)
...
async function completeTask(task) {
  try {
    ...
    if (props.showNotification) {
      props.showNotification('完成日常任务失败: ' + (error.message || '请求失败'), 'error')
    }
  } finally {
...
async function cancelCompleteTask(task) {
  try {
    ...
    if (props.showNotification) {
      props.showNotification('取消完成日常任务失败: ' + (error.message || '请求失败'), 'error')
    }
  } finally {
    if (task) task.isUpdating = false
  }
