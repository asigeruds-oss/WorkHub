<template>
  <v-card class="feedback-list">
    ...
    <v-card-text v-else-if="!feedbackList.length" class="text-center py-8">
      <v-icon size="64" color="grey-400" class="mb-4">mdi-comment-outline</v-icon>
            ...
            <div class="flex-grow-1">
              ...
              <div class="d-flex align-center justify-space-between">
                <v-btn 
                  ...
                  :prepend-icon="expandedReplies.has(feedback.id) ? 'mdi-chevron-up' : 'mdi-chevron-down'"
                >
              ...
              <!-- 回复列表 -->
              <v-expand-transition>
                <div v-if="expandedReplies.has(feedback.id)" class="replies-section mt-4">
                  <v-divider class="mb-3"></v-divider>
                  
                  <div v-if="!feedback.replies?.length" class="text-center py-4">
                  ...
                  <div v-else class="replies-list">
                    ...
                    >
                      <div class="d-flex align-center justify-space-between mb-2">
                        <div class="d-flex align-center gap-2">
                          <v-avatar size="24" :color="reply.is_admin ? 'orange' : 'blue'">
        ...
        <v-card-text class="pt-4">
          ...
          <div class="original-feedback pa-3 mb-4 rounded bg-grey-100">
            <div class="d-flex align-center mb-2">
              ...
              <span class="ml-2 font-weight-medium">{{ selectedFeedback?.name || '匿名用户' }}</span>
            </div>
            <div class="text-body-2">{{ selectedFeedback?.content }}</div>
          </div>
          
          <!-- 回复表单 -->
          <v-form ref="replyForm" v-model="replyValid" @submit.prevent="submitReply">
            <v-row>
              <v-col cols="12" md="6">
                <v-text-field
        ...
        replies: [
          {
            id: 1,
            author: '管理员',
            content: '感谢您的建议！我们正在开发更多模板，预计下个版本会新增5-8个模板供选择。',
            is_admin: true,
        ...
        replies: [
          {
            id: 2,
            author: '管理员',
            content: '为了保护用户隐私，我们不会保存任何个人信息。所有数据都是临时处理，生成后即清除。',
            is_admin: true,
...
async function submitReply() {
  ...
  try {
    ...
    const response = await FeedbackAPI.replyFeedback(selectedFeedback.value.id, {
